...
// ── Server-side farm snapshot (คำนวณ drop loot diff เอง ไม่พึ่ง client) ──
let _srvFarmSnapshot  = null;  // flat inventory ณ จุดเริ่ม session { itemName: qty }
let _srvFarmDiff      = {};    // accumulated diff ที่คำนวณจาก inventory จริง
let _srvFarmStartTime = null;

// flatten inventory object → { itemName: totalQty }
...
// คำนวณ diff (ค่าที่เพิ่มขึ้นเท่านั้น) จาก snapshot → current
function calcInvDiff(snapFlat, currentFlat) {
  const diff = {};
  for (const [name, qty] of Object.entries(currentFlat)) {
    const gain = qty - (snapFlat[name] || 0);
    if (gain > 0) diff[name] = gain;
  }
  return diff;
}

// อัปเดต server-side farm diff ทุกครั้งที่ inventory เปลี่ยน
function updateServerFarmDiff(newInventory) {
  const currentFlat = flattenInv(newInventory);

  // ถ้ายังไม่มี snapshot → ตั้งต้น
  if (!_srvFarmSnapshot) {
    _srvFarmSnapshot  = currentFlat;
    _srvFarmDiff      = {};
    _srvFarmStartTime = new Date().toISOString();
    console.log('[FARM-SRV] 📸 Snapshot ตั้งต้น');
    return;
  }

  // คำนวณ diff สะสมจาก snapshot
  _srvFarmDiff = calcInvDiff(_srvFarmSnapshot, currentFlat);
}
...
async function bootLoad() {
  ...
  try {
    ...
    if (fSess) {
      Object.assign(farmSession, fSess);
      // restore server-side snapshot จาก farmSession ที่โหลดมา
      if (fSess.snapshot && typeof fSess.snapshot === 'object') {
        _srvFarmSnapshot  = fSess.snapshot;
        _srvFarmDiff      = fSess.diff || {};
        _srvFarmStartTime = fSess.startTime || null;
        console.log('[FARM-SRV] 📸 Restored snapshot from Firestore');
      }
    }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && url === '/') {
    try {
      ...
      // ── อัปเดต server-side drop loot diff ทันที (ไม่พึ่ง client) ──
      updateServerFarmDiff(parsed);
  ...
  if (req.method === 'POST' && url === '/farm-session') {
    try {
      ...
      if (j.action === 'reset') {
        ...
        // reset server-side snapshot → take ใหม่จาก inventory ถัดไป
        _srvFarmSnapshot  = latestInventory ? flattenInv(latestInventory) : null;
        _srvFarmDiff      = {};
        _srvFarmStartTime = farmSession.startTime;
        console.log('[FARM-SRV] 🔄 Reset via client action=reset');
      } else if (j.action === 'start') {
        if (!farmSession.active) {
          farmSession = { snapshot: j.snapshot || null, diff: null, startTime: new Date().toISOString(), active: true };
          // take snapshot ใหม่ตอน start
          if (!_srvFarmSnapshot && latestInventory) {
            _srvFarmSnapshot  = flattenInv(latestInventory);
            _srvFarmDiff      = {};
            _srvFarmStartTime = farmSession.startTime;
            console.log('[FARM-SRV] 📸 Snapshot on start');
          }
  ...
  if (req.method === 'POST' && url === '/db-reset') {
    try {
      ...
      if (target === 'farm' || target === 'all') {
        ...
        // reset server-side snapshot ด้วย → จะ take snapshot ใหม่จาก inventory ถัดไป
        _srvFarmSnapshot  = null;
        _srvFarmDiff      = {};
        _srvFarmStartTime = null;
        console.log('[FARM-SRV] 🔄 Server farm snapshot reset');
...
function serverBuildEmbed(cfg) {
  ...
  // Farm diff — ใช้ _srvFarmDiff ที่ server คำนวณเองจาก inventory จริง (ไม่พึ่ง client)
  // ถ้า _srvFarmDiff ยังว่าง ให้ fallback ไป farmSession.diff ที่ client ส่งมา
  const farmDiff   = (Object.keys(_srvFarmDiff).length > 0) ? _srvFarmDiff : (farmSession.diff || {});
